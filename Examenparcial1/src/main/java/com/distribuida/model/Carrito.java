...
public class Carrito {
    ...
    // Opcional: permitir carrito para no autenticados
    ...
    // Totales (usamos BigDecimal para cálculo; si prefieres Double por tu esquema, cámbialos a Double)
    ...
    public void recomputarTotales(BigDecimal tasaIva) {
        // Subtotal: si el item.total es null, lo calculamos al vuelo
    ...
    public void recomprobacionTotalesCompat() {
        if (subtotal == null) subtotal = java.math.BigDecimal.ZERO;
        if (descuento == null) descuento = java.math.BigDecimal.ZERO;
        if (impuestos == null) impuestos = java.math.BigDecimal.ZERO;
